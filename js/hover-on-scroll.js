...
function handleScroll() {
  var figures = document.querySelectorAll('.effect-ming');
  var figuresTeam = document.querySelectorAll('.effect-sarah');
  ...
  figuresTeam.forEach(function (figure) {
    var rect = figure.getBoundingClientRect();
    var elementTop = rect.top;
    var elementBottom = rect.bottom;
    var elementCenter = (elementTop + elementBottom) / 2;
    ...
    // Check if the element is within the center range
    if (elementCenter > centerRangeStart && elementCenter < centerRangeEnd) {
      figure.classList.add('hovered-sarah');
    } else {
      figure.classList.remove('hovered-sarah');
    }
  });
